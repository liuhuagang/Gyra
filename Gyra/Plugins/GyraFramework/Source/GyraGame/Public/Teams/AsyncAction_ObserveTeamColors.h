...
class IGyraTeamAgentInterface;
class UGyraTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UGyraTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements IGyraTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UGyraTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UGyraTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<IGyraTeamAgentInterface> TeamInterfacePtr;
