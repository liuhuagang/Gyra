...
#include "GyraSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EGyraAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UGyraSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UGyraSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UGyraSettingsShared, FOnSettingChangedEvent, UGyraSettingsShared* Settings);
...
public:
	...
	static UGyraSettingsShared* CreateTemporarySettings(const UGyraLocalPlayer* LocalPlayer);

	/** Synchronously loads a settings object, this is not valid to call before login */
	static UGyraSettingsShared* LoadOrCreateSettings(const UGyraLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UGyraSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UGyraLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EGyraAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EGyraAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EGyraAllowBackgroundAudioSetting AllowAudioInBackground = EGyraAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EGyraGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EGyraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EGyraGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EGyraGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();

private:
	UPROPERTY()
	EGyraGamepadSensitivity GamepadLookSensitivityPreset = EGyraGamepadSensitivity::Normal;
	UPROPERTY()
	EGyraGamepadSensitivity GamepadTargetingSensitivityPreset = EGyraGamepadSensitivity::Normal;
