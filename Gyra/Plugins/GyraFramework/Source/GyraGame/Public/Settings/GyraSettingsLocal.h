...
#include "Input/GyraMappableConfigPair.h"
#include "InputCoreTypes.h"

#include "GyraSettingsLocal.generated.h"
...
USTRUCT()
struct FGyraScalabilitySnapshot
{
	GENERATED_BODY()

	FGyraScalabilitySnapshot();
 ...
 */
UCLASS()
class UGyraSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EGyraStatDisplayMode GetPerfStatDisplayState(EGyraDisplayablePerformanceStat Stat) const;

	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EGyraDisplayablePerformanceStat Stat, EGyraStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UGyraSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EGyraDisplayablePerformanceStat, EGyraStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FGyraScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FGyraScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UGyraSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
...
public:
	...
	DECLARE_EVENT_OneParam(UGyraSettingsLocal, FInputConfigDelegate, const FLoadedMappableConfigPair& /*Config*/);
	...
	UE_DEPRECATED(5.3, "AddOrUpdateCustomKeyboardBindings has been deprecated in favor of Enhanced Input User Settings")
		void AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UGyraLocalPlayer* LocalPlayer);
	...
	UE_DEPRECATED(5.3, "ResetKeybindingToDefault has been deprecated in favor of Enhanced Input User Settings")
		void ResetKeybindingToDefault(const FName MappingName, UGyraLocalPlayer* LocalPlayer);
	...
	UE_DEPRECATED(5.3, "ResetKeybindingsToDefault has been deprecated in favor of Enhanced Input User Settings")
		void ResetKeybindingsToDefault(UGyraLocalPlayer* LocalPlayer);
