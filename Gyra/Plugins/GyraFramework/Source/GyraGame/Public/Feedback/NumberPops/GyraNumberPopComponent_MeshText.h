...
#include "GyraNumberPopComponent_MeshText.generated.h"

class UGyraDamagePopStyle;
...
class UGyraNumberPopComponent_MeshText : public UGyraNumberPopComponent
{
	GENERATED_BODY()

public:

	UGyraNumberPopComponent_MeshText(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	void SetMaterialParameters(const FGyraNumberPopRequest& Request, FTempNumberPopInfo& NewDamageNumberInfo, const FTransform& CameraTransform, const FVector& NumberLocation);

	FLinearColor DetermineColor(const FGyraNumberPopRequest& Request) const;
	UStaticMesh* DetermineStaticMesh(const FGyraNumberPopRequest& Request) const;
	...
	TArray<TObjectPtr<UGyraDamagePopStyle>> Styles;
