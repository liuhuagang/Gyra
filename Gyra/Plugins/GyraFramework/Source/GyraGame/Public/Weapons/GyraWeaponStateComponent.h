...
#include "GyraWeaponStateComponent.generated.h"
...
struct FGyraScreenSpaceHitLocation
{
...
struct FGyraServerSideHitMarkerBatch
{
	FGyraServerSideHitMarkerBatch() { }

	FGyraServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FGyraScreenSpaceHitLocation> Markers;
...
UCLASS()
class UGyraWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UGyraWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FGyraScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FGyraScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FGyraServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
