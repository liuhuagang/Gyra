...
#include "GyraGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UGyraRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EGyraAbilityTargetingSource : uint8
{
...
/**
 * UGyraGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UGyraGameplayAbility_RangedWeapon : public UGyraGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UGyraGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Gyra|Ability")
	UGyraRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UGyraRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EGyraAbilityTargetingSource Source) const;
