...
#include "GyraPlayerStart.generated.h"

class AController;
class UObject;

enum class EGyraPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
 ...
 */
UCLASS(Config = Game)
class GYRAGAME_API AGyraPlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:
	AGyraPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	EGyraPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
