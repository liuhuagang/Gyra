...
#include "GyraPlayerState.generated.h"
...
UENUM()
enum class EGyraPlayerConnectionType : uint8
{
	// An active player
	Player = 0,
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FGyraMutableDelegate);
 ...
 */
UCLASS(Config = Game)
class GYRAGAME_API AGyraPlayerState : public AModularPlayerState, public IAbilitySystemInterface, public IGyraTeamAgentInterface
{
	GENERATED_BODY()
...
public:
	...
	void ClientCallServerUpdateMutableInfo(FGyraMutableInfo InMutableInfo);

	UFUNCTION(BlueprintCallable )
	FGyraMutableInfo GetMutableInfo( );


	AGyraPlayerState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category = "Gyra|PlayerState")
	...
	static const FName NAME_GyraAbilityReady;

	void SetPlayerConnectionType(EGyraPlayerConnectionType NewType);
	EGyraPlayerConnectionType GetPlayerConnectionType() const { return MyPlayerConnectionType; }
	...
	UFUNCTION(Client, Unreliable, BlueprintCallable, Category = "Gyra|PlayerState")
	void ClientBroadcastMessage(const FGyraVerbMessage Message);
...
private:
	void OnExperienceLoaded(const UGyraExperienceDefinition* CurrentExperience);
...
public:
	...
	FGyraMutableDelegate MutableDelegate;
...
protected:
	...
	UPROPERTY(ReplicatedUsing = OnRep_MutableInfo)
...
private:
	...
	// MagicSet attribute set used by this actor.
	UPROPERTY()
	TObjectPtr<const class UGyraMagicSet> MagicSet;
	
	// StatsSet attribute set used by this actor.
	UPROPERTY()
	TObjectPtr<const class UGyraStatsSet> StatsSet;

	UPROPERTY(Replicated)
	EGyraPlayerConnectionType MyPlayerConnectionType;
