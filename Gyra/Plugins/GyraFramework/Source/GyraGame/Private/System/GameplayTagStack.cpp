...
TArray<FGyraInventoryTagStack> FGameplayTagStackContainer::GetArchiveTagArray()
{
	TArray<FGyraInventoryTagStack> TagStacks;
	for (auto &TmpTag : Stacks)
	{
		FGyraInventoryTagStack& TagStack= TagStacks.AddDefaulted_GetRef();

		TagStack.AttributeTag = TmpTag.Tag;
		TagStack.AttributeValue= TmpTag.StackCount;
