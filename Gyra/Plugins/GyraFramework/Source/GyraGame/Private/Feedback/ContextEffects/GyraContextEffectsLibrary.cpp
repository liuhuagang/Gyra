...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraContextEffectsLibrary)


void UGyraContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UGyraContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UGyraContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UGyraContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FGyraContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UGyraActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FGyraContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UGyraActiveContextEffects* NewActiveContextEffects = NewObject<UGyraActiveContextEffects>(this);
	...
	this->GyraContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UGyraContextEffectsLibrary::GyraContextEffectLibraryLoadingComplete(
	TArray<UGyraActiveContextEffects*> GyraActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(GyraActiveContextEffects);
}
