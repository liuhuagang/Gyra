...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraContextEffectsSubsystem)
...
void UGyraContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UGyraContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UGyraContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UGyraContextEffectsLibrary* EffectLibrary : EffectsLibraries->GyraContextEffectsLibraries)
			{
...
bool UGyraContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UGyraContextEffectsSettings* ProjectSettings = GetDefault<UGyraContextEffectsSettings>())
	{
...
void UGyraContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UGyraContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UGyraContextEffectsSet* EffectsLibrariesSet = NewObject<UGyraContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UGyraContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UGyraContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->GyraContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UGyraContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
