...
#include "AnimNotify_GyraContextEffects.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_GyraContextEffects)



UAnimNotify_GyraContextEffects::UAnimNotify_GyraContextEffects()
{
}

void UAnimNotify_GyraContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_GyraContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_GyraContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_GyraContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> GyraContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UGyraContextEffectsInterface>())
			{
				// If so, add it to the Array
				GyraContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UGyraContextEffectsInterface>())
					{
						GyraContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* GyraContextEffectImplementingObject : GyraContextEffectImplementingObjects)
			{
				if (GyraContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IGyraContextEffectsInterface::Execute_AnimMotionEffect(GyraContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UGyraContextEffectLibrary type
						if (UGyraContextEffectsLibrary* EffectLibrary = Cast<UGyraContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_GyraContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_GyraContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
