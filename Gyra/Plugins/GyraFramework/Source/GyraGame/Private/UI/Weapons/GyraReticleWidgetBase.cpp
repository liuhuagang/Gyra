...
#include "GyraReticleWidgetBase.h"

#include "GyraInventoryItemInstance.h"
#include "GyraRangedWeaponInstance.h"
#include "GyraWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraReticleWidgetBase)

UGyraReticleWidgetBase::UGyraReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UGyraReticleWidgetBase::InitializeFromWeapon(UGyraWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UGyraInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UGyraReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UGyraRangedWeaponInstance* RangedWeapon = Cast<const UGyraRangedWeaponInstance>(WeaponInstance))
	{
...
bool UGyraReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UGyraRangedWeaponInstance* RangedWeapon = Cast<const UGyraRangedWeaponInstance>(WeaponInstance))
	{
...
float UGyraReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
