...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraControllerDisconnectedScreen)
...
UGyraControllerDisconnectedScreen::UGyraControllerDisconnectedScreen(const FObjectInitializer& ObjectInitializer)
...
void UGyraControllerDisconnectedScreen::NativeOnActivated()
{
	Super::NativeOnActivated();

	if (!HBox_SwitchUser)
	{
		UE_LOG(LogGyraGame, Error, TEXT("Unable to find HBox_SwitchUser on Widget %s"), *GetNameSafe(this));
		return;
	}

	if (!Button_ChangeUser)
	{
		UE_LOG(LogGyraGame, Error, TEXT("Unable to find Button_ChangeUser on Widget %s"), *GetNameSafe(this));
		return;
	}
...
bool UGyraControllerDisconnectedScreen::ShouldDisplayChangeUserButton() const
{
...
void UGyraControllerDisconnectedScreen::HandleChangeUserClicked()
{
	ensure(ShouldDisplayChangeUserButton());

	UE_LOG(LogGyraGame, Log, TEXT("[%hs] Change user requested!"), __func__);
...
void UGyraControllerDisconnectedScreen::HandleChangeUserCompleted(const FPlatformUserSelectionCompleteParams& Params)
{
	UE_LOG(LogGyraGame, Log, TEXT("[%hs] User change complete!"), __func__);
