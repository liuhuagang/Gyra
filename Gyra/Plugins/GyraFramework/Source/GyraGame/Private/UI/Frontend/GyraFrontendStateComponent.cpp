...
#include "GyraFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraFrontendStateComponent)
...
UGyraFrontendStateComponent::UGyraFrontendStateComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UGyraFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
...
void UGyraFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UGyraFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UGyraFrontendStateComponent::OnExperienceLoaded(const UGyraExperienceDefinition* Experience)
{
...
void UGyraFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UGyraFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UGyraFrontendStateComponent::OnUserInitialized);
...
void UGyraFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
                                                    ...
                                                    ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UGyraFrontendStateComponent::OnUserInitialized);
...
void UGyraFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UGyraFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
