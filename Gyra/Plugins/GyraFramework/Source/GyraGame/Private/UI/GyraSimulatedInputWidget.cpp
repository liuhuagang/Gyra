...
#include "GyraSimulatedInputWidget.h"
#include "EnhancedInputSubsystems.h"
#include "GenericPlatform/GenericPlatformInputDeviceMapper.h"
#include "LogGyraGame.h"
#include "InputKeyEventArgs.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraSimulatedInputWidget)

#define LOCTEXT_NAMESPACE "GyraSimulatedInputWidget"

UGyraSimulatedInputWidget::UGyraSimulatedInputWidget(const FObjectInitializer& ObjectInitializer)
...
#if WITH_EDITOR
const FText UGyraSimulatedInputWidget::GetPaletteCategory()
{
	return LOCTEXT("PalleteCategory", "Input");
}
#endif // WITH_EDITOR

void UGyraSimulatedInputWidget::NativeConstruct()
{
	...
	{
		System->ControlMappingsRebuiltDelegate.AddUniqueDynamic(this, &UGyraSimulatedInputWidget::OnControlMappingsRebuilt);
	}
	
	Super::NativeConstruct();
}

void UGyraSimulatedInputWidget::NativeDestruct()
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
FReply UGyraSimulatedInputWidget::NativeOnTouchEnded(const FGeometry& InGeometry, const FPointerEvent& InGestureEvent)
{
	FlushSimulatedInput();
...
UEnhancedInputLocalPlayerSubsystem* UGyraSimulatedInputWidget::GetEnhancedInputSubsystem() const
{
...
UEnhancedPlayerInput* UGyraSimulatedInputWidget::GetPlayerInput() const
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
void UGyraSimulatedInputWidget::InputKeyValue(const FVector& Value)
{
	...
	{
		UE_LOG(LogGyraGame, Error, TEXT("'%s' is attempting to simulate input but has no player input!"), *GetNameSafe(this));
	}
}

void UGyraSimulatedInputWidget::InputKeyValue2D(const FVector2D& Value)
{
	InputKeyValue(FVector(Value.X, Value.Y, 0.0));
}

void UGyraSimulatedInputWidget::FlushSimulatedInput()
{
...
void UGyraSimulatedInputWidget::QueryKeyToSimulate()
{
	if (UEnhancedInputLocalPlayerSubsystem* System = GetEnhancedInputSubsystem())
	{
...
void UGyraSimulatedInputWidget::OnControlMappingsRebuilt()
{
	QueryKeyToSimulate();
}
