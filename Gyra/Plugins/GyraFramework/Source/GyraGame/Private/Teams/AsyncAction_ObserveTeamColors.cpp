...
UAsyncAction_ObserveTeamColors* UAsyncAction_ObserveTeamColors::ObserveTeamColors(UObject* TeamAgent)
{
	...
	if (TeamAgent != nullptr)
	{
		...
		Action->TeamInterfacePtr = TWeakInterfacePtr<IGyraTeamAgentInterface>(TeamAgent);
...
void UAsyncAction_ObserveTeamColors::SetReadyToDestroy()
{
	...
	if (IGyraTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeamColors::Activate()
{
	...
	UGyraTeamDisplayAsset* CurrentDisplayAsset = nullptr;
	...
	if (IGyraTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull))
		{
			...
			CurrentDisplayAsset = UGyraTeamStatics::GetTeamDisplayAsset(World, CurrentTeamIndex);
...
void UAsyncAction_ObserveTeamColors::BroadcastChange(int32 NewTeam, const UGyraTeamDisplayAsset* DisplayAsset)
{
	UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull);
	UGyraTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UGyraTeamSubsystem>(World);
...
{
	UGyraTeamDisplayAsset* DisplayAsset = UGyraTeamStatics::GetTeamDisplayAsset(TeamAgent, NewTeam);
	BroadcastChange(NewTeam, DisplayAsset);
}

void UAsyncAction_ObserveTeamColors::OnDisplayAssetChanged(const UGyraTeamDisplayAsset* DisplayAsset)
{
