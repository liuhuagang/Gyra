...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraExperienceDefinition)

#define LOCTEXT_NAMESPACE "GyraSystem"

UGyraExperienceDefinition::UGyraExperienceDefinition()
{
...
EDataValidationResult UGyraExperienceDefinition::IsDataValid(FDataValidationContext& Context) const
{
	//合并两个不同的数据验证结果并返回合并后的结果。
	...
	// 确保用户没有从这个BP中子类化（这很好，并且期望在BP中子类化一次，而不是两次）
...
void UGyraExperienceDefinition::UpdateAssetBundleData()
{
