...
#include "GyraInputModifiers.h"

#include "EnhancedPlayerInput.h"
#include "GameFramework/PlayerController.h"
#include "Input/GyraAimSensitivityData.h"
#include "Player/GyraLocalPlayer.h"
#include "GyraSettingsShared.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogGyraInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// GyraInputModifiersHelpers

namespace GyraInputModifiersHelpers
{
	/** Returns the owning GyraLocalPlayer of an Enhanced Player Input pointer */
	static UGyraLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UGyraLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UGyraSettingBasedScalar

FInputActionValue UGyraSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UGyraLocalPlayer* LocalPlayer = GyraInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UGyraSettingsShared::StaticClass();
			UGyraSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UGyraInputModifierDeadZone

FInputActionValue UGyraInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UGyraLocalPlayer* LocalPlayer = GyraInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UGyraSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UGyraInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UGyraInputModifierGamepadSensitivity

FInputActionValue UGyraInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EGyraGamepadSensitivity Sensitivity = (TargetingType == EGyraTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UGyraInputModifierAimInversion

FInputActionValue UGyraInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UGyraLocalPlayer* LocalPlayer = GyraInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
