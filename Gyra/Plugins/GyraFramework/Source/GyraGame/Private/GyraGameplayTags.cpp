...
namespace GyraGameplayTags
{
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(InitState_DataPrepared, "InitState.DataPrepared", "2: Actor/component has initially spawned and can be extended.Millde Temp Optional");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(InitState_DataAvailable, "InitState.DataAvailable", "3: All required data has been loaded/replicated and is ready for initialization");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(InitState_DataInitialized, "InitState.DataInitialized", "4: The available data has been initialized for this actor/component, but it is not ready for full gameplay");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(InitState_GameplayReady, "InitState.GameplayReady", "5: The actor/component is fully ready for active gameplay");
	...
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Gyra_ShooterGame_Inventory_MaxStackCount, "Gyra.ShooterGame.Inventory.MaxStackCount", "The maximum number of stacked items of this type.");

	UE_DEFINE_GAMEPLAY_TAG_COMMENT(DamageEffect_Physics_NormalAttack, "DamageEffect.Physics.NormalAttack", "Ordinary physical attack.");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(DamageEffect_Physics_WeakAttack, "DamageEffect.Physics.WeakAttack", "Weakness attack.");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(DamageEffect_Physics_CriticalAttack, "DamageEffect.Physics.CriticalAttack", "Critical Hit.");
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(DamageEffect_Physics_DeadAttack, "DamageEffect.Physics.FatalAttack", "A fatal attack triggers both critical and weakness attacks.");
	...
	UE_DEFINE_GAMEPLAY_TAG(TAG_Gyra_Damage_Message, "Gyra.Damage.Message");
	UE_DEFINE_GAMEPLAY_TAG(TAG_Gyra_Elimination_Message, "Gyra.Elimination.Message");
	UE_DEFINE_GAMEPLAY_TAG(TAG_Gyra_Assist_Message, "Gyra.Assist.Message");
	UE_DEFINE_GAMEPLAY_TAG(TAG_Gyra_Cost_Message, "Gyra.Damage.Cost.Message");
	...
	// When extending Gyra, you can create your own movement modes but you need to update GetCustomMovementModeTagMap()
	UE_DEFINE_GAMEPLAY_TAG_COMMENT(Movement_Mode_Custom, "Movement.Mode.Custom", "This is invalid and should be replaced with custom tags.  See GyraGameplayTags::CustomMovementModeTagMap.");
	...
	FGameplayTag FindTagByString(const FString& TagString, bool bMatchPartialString)
	{
		...
		if (!Tag.IsValid() && bMatchPartialString)
		{
			...
			for (const FGameplayTag& TestTag : AllTags)
			{
				...
				{
					UE_LOG(LogGyraGame, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
					Tag = TestTag;
					break;
				}
			}
