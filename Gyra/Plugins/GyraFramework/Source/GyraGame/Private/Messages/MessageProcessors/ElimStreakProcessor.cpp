...
void UElimStreakProcessor::StartListening()
{
	...
	AddListenerHandle(MessageSubsystem.RegisterListener(ElimStreak::TAG_Gyra_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimStreakProcessor::OnEliminationMessage(FGameplayTag Channel, const FGyraVerbMessage& Payload)
{
	...
	{ 
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			{
				FGyraVerbMessage ElimStreakMessage;
