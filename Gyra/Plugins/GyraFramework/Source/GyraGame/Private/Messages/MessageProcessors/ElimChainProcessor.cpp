...
void UElimChainProcessor::StartListening()
{
	...
	AddListenerHandle(MessageSubsystem.RegisterListener(ElimChain::TAG_Gyra_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimChainProcessor::OnEliminationMessage(FGameplayTag Channel, const FGyraVerbMessage& Payload)
{
	...
	if (Payload.Instigator != Payload.Target)
	{
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			else
			{
				...
				{
					FGyraVerbMessage ElimChainMessage;
