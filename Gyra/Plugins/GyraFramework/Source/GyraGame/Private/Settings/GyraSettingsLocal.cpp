...
#include "Development/GyraPlatformEmulationSettings.h"
#include "SoundControlBus.h"
#include "AudioModulationStatics.h"
#include "Audio/GyraAudioSettings.h"
#include "Audio/GyraAudioMixEffectsSubsystem.h"
#include "EnhancedActionKeyMapping.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Gyra.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Gyra.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Gyra.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Gyra.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Gyra.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Gyra.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Gyra.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Gyra.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Gyra.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Gyra.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FGyraScalabilitySnapshot::FGyraScalabilitySnapshot()
{
...
namespace GyraSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FGyraScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
			{
				...
				const bool bIsSupported = UGyraSettingsLocal::IsSupportedMobileFramePace(TestRate);

				return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
			});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UGyraSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UGyraSettingsLocal::UGyraSettingsLocal()
{
...
void UGyraSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UGyraSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	GyraSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UGyraSettingsLocal::ResetToCurrentSettings()
{
...
void UGyraSettingsLocal::BeginDestroy()
{
...
UGyraSettingsLocal* UGyraSettingsLocal::Get()
{
	return GEngine ? CastChecked<UGyraSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UGyraSettingsLocal::ConfirmVideoMode()
{
...
float UGyraSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::DesktopStyle)
	{
...
int32 UGyraSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return GyraSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UGyraSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FGyraScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UGyraSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UGyraSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGyraSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UGyraSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UGyraSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UGyraSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EGyraStatDisplayMode UGyraSettingsLocal::GetPerfStatDisplayState(EGyraDisplayablePerformanceStat Stat) const
{
	if (const EGyraStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EGyraStatDisplayMode::Hidden;
	}
}

void UGyraSettingsLocal::SetPerfStatDisplayState(EGyraDisplayablePerformanceStat Stat, EGyraStatDisplayMode DisplayMode)
{
	if (DisplayMode == EGyraStatDisplayMode::Hidden)
	{
...
float UGyraSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UGyraSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UGyraSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UGyraSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UGyraSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGyraSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UGyraSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGyraSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UGyraSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UGyraSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UGyraSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UGyraSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UGyraSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UGyraSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UGyraSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UGyraSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return GyraSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UGyraSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return GyraSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UGyraSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UGyraSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EGyraFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return GyraSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UGyraSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::MobileStyle)
	{
...
void UGyraSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UGyraSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = GyraSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UGyraSettingsLocal::ClampMobileQuality()
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::MobileStyle)
	{
...
void UGyraSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = GyraSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UGyraSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = GyraSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = GyraSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UGyraSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UGyraSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UGyraSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UGyraSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UGyraSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = GyraSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UGyraSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UGyraSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UGyraAudioMixEffectsSubsystem* GyraAudioMixEffectsSubsystem = World->GetSubsystem<UGyraAudioMixEffectsSubsystem>())
			{
				GyraAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UGyraSettingsLocal::CanRunAutoBenchmark() const
{
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UGyraSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UGyraSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UGyraSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UGyraSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UGyraSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UGyraSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UGyraSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGyraSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UGyraSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGyraSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UGyraSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UGyraSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UGyraSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UGyraSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UGyraSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UGyraSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UGyraSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UGyraSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UGyraSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EGyraFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = GyraSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UGyraSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UGyraSettingsLocal::GetControllerPlatform() const
{
...
void UGyraSettingsLocal::RegisterInputConfig(ECommonInputType Type, const UPlayerMappableInputConfig* NewConfig, const bool bIsActive)
{
	if (NewConfig)
	{
...
int32 UGyraSettingsLocal::UnregisterInputConfig(const UPlayerMappableInputConfig* ConfigToRemove)
{
	if (ConfigToRemove)
	{
...
const UPlayerMappableInputConfig* UGyraSettingsLocal::GetInputConfigByName(FName ConfigName) const
{
...
void UGyraSettingsLocal::GetRegisteredInputConfigsOfType(ECommonInputType Type, TArray<FLoadedMappableConfigPair>& OutArray) const
{
	OutArray.Empty();
...
void UGyraSettingsLocal::GetAllMappingNamesFromKey(const FKey InKey, TArray<FName>& OutActionNames)
{
...
void UGyraSettingsLocal::AddOrUpdateCustomKeyboardBindings(const FName MappingName, const FKey NewKey, UGyraLocalPlayer* LocalPlayer)
{
...
void UGyraSettingsLocal::ResetKeybindingToDefault(const FName MappingName, UGyraLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UGyraSettingsLocal::ResetKeybindingsToDefault(UGyraLocalPlayer* LocalPlayer)
{
	if (UEnhancedInputLocalPlayerSubsystem* Subsystem = ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(LocalPlayer))
	{
...
void UGyraSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UGyraAudioSettings* GyraAudioSettings = GetDefault<UGyraAudioSettings>())
			{
				...
				if (UObject* ObjPath = GyraAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Gyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = GyraAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Gyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = GyraAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Gyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = GyraAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Gyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = GyraAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Gyra Audio Settings."));
					}
				}

				if (UObject* ObjPath = GyraAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Gyra Audio Settings."));
					}
				}
			}
		}
...
void UGyraSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UGyraSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UGyraPlatformSpecificRenderingSettings* PlatformSettings = UGyraPlatformSpecificRenderingSettings::Get();
	const TArray<FGyraQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FGyraQualityDeviceProfileVariant& Data) { return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UGyraPlatformEmulationSettings* Settings = GetDefault<UGyraPlatformEmulationSettings>();
	...
	{
	case EGyraFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EGyraFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EGyraFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UGyraSettingsLocal::UpdateConsoleFramePacing()
{
...
void UGyraSettingsLocal::UpdateDesktopFramePacing()
{
...
void UGyraSettingsLocal::UpdateMobileFramePacing()
{
...
void UGyraSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
