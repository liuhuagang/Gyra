...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraSafeZoneEditor)
...
UGyraSafeZoneEditor::UGyraSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UGyraSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UGyraSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UGyraSettingsLocal::Get()->GetSafeZone());

	Button_Done->OnClicked().AddUObject(this, &UGyraSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel) ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UGyraSafeZoneEditor::HandleBackClicked);
	}
}

bool UGyraSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UGyraSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UGyraSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UGyraSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UGyraSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UGyraSettingsLocal::Get()->GetSafeZone());
}

void UGyraSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UGyraSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
