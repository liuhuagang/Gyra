...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gyra_Weapon_SteadyAimingCamera, "Gyra.Weapon.SteadyAimingCamera");

UGyraRangedWeaponInstance::UGyraRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UGyraRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UGyraRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UGyraRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UGyraRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UGyraRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UGyraRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UGyraRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UGyraRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UGyraRangedWeaponInstance::AddSpread()
{
...
float UGyraRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UGyraRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UGyraRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UGyraRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UGyraCameraComponent* CameraComponent = UGyraCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_Gyra_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
