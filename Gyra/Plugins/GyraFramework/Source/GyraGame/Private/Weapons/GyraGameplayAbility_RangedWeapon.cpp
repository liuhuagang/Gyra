...
#include "GyraGameplayAbility_RangedWeapon.h"
#include "GyraRangedWeaponInstance.h"
#include "Physics/GyraCollisionChannels.h"
#include "LogGyraGame.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "GyraWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/GyraGameplayAbilityTargetData_SingleTargetHit.h"


#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraGameplayAbility_RangedWeapon)
...
namespace GyraConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("Gyra.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("Gyra.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("Gyra.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UGyraGameplayAbility_RangedWeapon::UGyraGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UGyraRangedWeaponInstance* UGyraGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UGyraRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UGyraGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogGyraAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UGyraRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UGyraGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UGyraGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UGyraGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Gyra_TraceChannel_Weapon;
}

FHitResult UGyraGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UGyraGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UGyraGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EGyraAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UGyraWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UGyraWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EGyraAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EGyraAbilityTargetingSource::CameraTowardsFocus) || (Source == EGyraAbilityTargetingSource::PawnTowardsFocus) || (Source == EGyraAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EGyraAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EGyraAbilityTargetingSource::WeaponForward) || (Source == EGyraAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EGyraAbilityTargetingSource::PawnTowardsFocus) || (Source == EGyraAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UGyraGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (GyraConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, GyraConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UGyraGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UGyraRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EGyraAbilityTargetingSource::CameraTowardsFocus);
		...
		if (GyraConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, GyraConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UGyraGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UGyraRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (GyraConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, GyraConsoleVariables::DrawBulletHitRadius, FColor::Red, false, GyraConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UGyraGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UGyraGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UGyraGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UGyraWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UGyraWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogGyraAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UGyraGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UGyraWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UGyraWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FGyraGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FGyraGameplayAbilityTargetData_SingleTargetHit();
