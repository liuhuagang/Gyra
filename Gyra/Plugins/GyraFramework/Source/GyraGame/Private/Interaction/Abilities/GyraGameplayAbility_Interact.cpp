...
#include "GyraGameplayAbility_Interact.h"
...
#include "UI/IndicatorSystem/GyraIndicatorManagerComponent.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GyraGameplayAbility_Interact)
...
UGyraGameplayAbility_Interact::UGyraGameplayAbility_Interact(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ActivationPolicy = EGyraAbilityActivationPolicy::OnSpawn;
	InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
	NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;
}

void UGyraGameplayAbility_Interact::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UGyraGameplayAbility_Interact::UpdateInteractions(const TArray<FInteractionOption>& InteractiveOptions)
{
	if (AGyraPlayerController* PC = GetGyraPlayerControllerFromActorInfo())
	{
		if (UGyraIndicatorManagerComponent* IndicatorManager = UGyraIndicatorManagerComponent::GetComponent(PC))
		{
...
void UGyraGameplayAbility_Interact::TriggerInteraction()
{
